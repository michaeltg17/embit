...
TAP_SIGS = [
    (
        unhexlify(
            "6f7f1255071fb5a103b5a4d3e5e295d19e9701e58fa1c457e92733c53ed16804f1036c90f30f6c4753a884c2be8b7d4a7c30a2a86dbfb0e8010bdf7064fd70f7"
        ),  # sig of keyA
    ),
    (
        unhexlify(
            "0574a2735988c4c8bd866ac546ae3f8a29f19a9596742892638c53f3b593269975135f0c58b2db945723e109db4111f6789e2abd37d2de82e94b8af47de63bda"
        ),  # A
    ),
]
...
TAPTREE_SIGS = [
    unhexlify(
        "fd2877b89797991d0e237f466a917a5300944f6fb1453b576763ef76e5d1140816d4ec923b36975b859cb744a85286cd0d15a6f7c0ed15f9d03852c7716b3207"
    ),  # internal
    unhexlify(
        "5fd88ed2ce431b003ce2b0e233a3b7870514b130e700b612a4d86fedeb603657ac88e2f4f290ee7ed16e23f6d27f717a5447acdbea5e72d16497a29a4ea46ca6"
    ),  # keyA
    unhexlify(
        "fe9f994315faa9caab4e41d78b0200ca19fd3c735ce7d88e879b0da0b7e1f4dd31f877b6529700f463670f1a6f6ea217f6dfc596cf8c55e78b42b70e3491aac3"
    ),  # keyB
    unhexlify(
        "d09a024358e7f609a0c3dfb94ac852c3f20859784f18911b5ec4cbc8c5ebf4a3642254519b47368fa1b2df75e10c56b77e7f9f34157335c2bc814106e544077c"
    ),  # keyC
]
...
class TaprootTest(TestCase):
    ...
    def test_sign_verify(self):
        ...
        for i, inp in enumerate(signed.inputs):
            ...
            hsh = tx.sighash_taproot(
                i, script_pubkeys=scripts, values=values, sighash=sighash
            )
            ...
            self.assertEqual(
                unsigned.inputs[i].final_scriptwitness,
    ...
    def test_sighash_taproot_single_anyonecanpay(self):
        """BIP341 SIGHASH_SINGLE | SIGHASH_ANYONECANPAY takes the separate
        ANYONECANPAY input path (outpoint | amount | scriptPubKey | nSequence)
        but still embeds ``sha_single_output`` = SHA256 of the corresponding
        ...
        # Independently build the BIP341 SigMsg for a key-path
        # SIGHASH_SINGLE | SIGHASH_ANYONECANPAY spend (spend_type=0, no annex,
        # no leaf). The ANYONECANPAY input data is:
        #   outpoint (36) | amount (8) | scriptPubKey (compact size + script)
        #   | nSequence (4)
        sigmsg = (
            bytes([SIGHASH.SINGLE | SIGHASH.ANYONECANPAY])
            + tx.version.to_bytes(4, "little")
            + tx.locktime.to_bytes(4, "little")
            + bytes([0])  # spend_type (key-path, no annex)
            + bytes(reversed(tx.vin[0].txid))  # outpoint: prevout hash (32)
            + tx.vin[0].vout.to_bytes(4, "little")  # outpoint: prevout index (4)
            + values[0].to_bytes(8, "little")  # amount (8)
            + script_pubkeys[0].serialize()  # scriptPubKey (compact size + script)
            + tx.vin[0].sequence.to_bytes(4, "little")  # nSequence (4)
            + hashes.sha256(tx.vout[0].serialize())  # sha_single_output
        )
        expected = hashes.tagged_hash("TapSighash", b"\x00" + sigmsg)
